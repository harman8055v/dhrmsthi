-- Enhance swipe trigger: when a reciprocal like exists, enqueue a match job for the original liker
...
begin
  if new.action in ('like','superlike') then
    -- Enqueue like/superlike (deduped per minute)
    ...
    -- Check reciprocal like and enqueue match to original liker
    ...
    if found then
      ...
      -- Dedupe key for match
      v_dedupe := 'match:' || v_user1::text || ':' || v_user2::text;
