...
export default function SafeHeaderProfileDropdown() {
  const pathname = usePathname()
  const router = useRouter()
  const [isProfileMenuOpen, setIsProfileMenuOpen] = useState(false)
  const dropdownRef = useRef<HTMLDivElement>(null)
  
  // Get auth context - always call hooks first before any conditional logic
  ...
  // Only render on dashboard pages and if user profile exists
  if (!pathname.startsWith("/dashboard") || !userProfile) {
    return null
  }
