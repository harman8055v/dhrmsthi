...
interface DemoCompatibilityData {
  total: number
  breakdown: {
    spiritual: number
    lifestyle: number
...
const demoProfile1 = {
  ...
  spiritual_org: ["ISKCON", "Art of Living"],
...
const demoProfile2 = {
  ...
  spiritual_org: ["ISKCON", "Brahma Kumaris"],
  daily_practices: ["Meditation", "Yoga", "Scripture Reading"],
  diet: "Vegetarian",
  temple_freq: "Daily",
  philosophy: "Moksha-focused",
  vanaprastha: "Yes",
  quote: "Peace comes from within. Do not seek it without. - Buddha"
}

const demoCompatibility: DemoCompatibilityData = {
  total: 94,
  breakdown: {
    spiritual: 96,
    lifestyle: 92,
  ...
  reasons: [
    ...
    "Compatible temple visit patterns for joint worship",
    "Mutual interest in Vanaprastha lifestyle stage",
    "Both quote Buddha - philosophical alignment"
  ],
  concerns: [
...
export default function MatchingDemo() {
  ...
  const categoryData = {
    spiritual: { name: 'Spiritual Harmony', icon: '🕉️', color: 'purple', description: 'Shared practices, beliefs, and spiritual journey' },
    lifestyle: { name: 'Lifestyle Sync', icon: '🌱', color: 'green', description: 'Daily habits, diet, and life patterns' },
    psychological: { name: 'Mindset Match', icon: '🧠', color: 'blue', description: 'Communication style and mental compatibility' },
    demographic: { name: 'Life Context', icon: '📍', color: 'orange', description: 'Age, location, and background alignment' },
    preference: { name: 'Desire Alignment', icon: '💕', color: 'pink', description: 'What you seek matches what they offer' },
    semantic: { name: 'Expression Harmony', icon: '💬', color: 'indigo', description: 'How you both express and communicate' },
    growth_potential: { name: 'Future Growth', icon: '🌟', color: 'yellow', description: 'Potential for mutual spiritual evolution' }
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-gray-600 mb-6">
            Experience the most sophisticated spiritual compatibility analysis in the world
          </p>
          <Button 
          ...
          <Card className="border-2 border-blue-200 bg-blue-50/50">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900">{demoProfile1.name}</h3>
                  <p className="text-sm text-gray-600">{demoProfile1.age} • {demoProfile1.profession}</p>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(demoProfile1.daily_practices) && demoProfile1.daily_practices.map((practice, i) => (
                    <Badge key={i} variant="outline" className="text-xs">{practice}</Badge>
                  ))}
                </div>
              </div>
          ...
          <Card className="border-2 border-pink-200 bg-pink-50/50">
            <CardHeader>
              <CardTitle className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900">{demoProfile2.name}</h3>
                  <p className="text-sm text-gray-600">{demoProfile2.age} • {demoProfile2.profession}</p>
                </div>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <MapPin className="w-4 h-4" />
                {demoProfile2.location}
              </div>
              <div>
                <p className="text-sm font-medium text-gray-700 mb-2">Spiritual Organizations</p>
                <div className="flex flex-wrap gap-2">
                  {demoProfile2.spiritual_org.map((org, i) => (
                    <Badge key={i} variant="secondary" className="text-xs">{org}</Badge>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-700 mb-2">Daily Practices</p>
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(demoProfile2.daily_practices) && demoProfile2.daily_practices.map((practice, i) => (
                    <Badge key={i} variant="outline" className="text-xs">{practice}</Badge>
                  ))}
                </div>
              </div>
        ...
        <AnimatePresence>
          {isAnimating && (
            ...
            >
              <div className="inline-flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-full text-lg font-bold shadow-lg">
